function [violation, violationbychoice, CKB, CKBmin, CKBbychoice] = threeItemGARP1213ViolationCounter(set, choice)
            ...
            if (set(1,1,i) ~= set(1,1,j) ||...   %Then is first basket in 'i' not equal to the first basket in 'j'
                ...
                if (set(2,1,i) ~= set(2,1,j) ||...%Then is second basket in 'i' not equal to the second basket in 'j'
                    ...
                    if choice(i) == 2 && ...
                        ...
                        right = [set(2,1,i),set(2,3,i);set(2,1,j),set(2,3,j)]; % defines the right bundles
                        dRight = pdist(right,'euclidean'); % computes the euclidean distance between the right bundles
                        if i ~= j
                            violation(i,j) = 1;
                            CKB(i,j) = (dLeft+dRight)/2;
                            CKBmin(i,j) = min(dLeft,dRight);
                        elseif i == j;
                            violation(i,j) = 2;
                            CKB(i,j) = dLeft+dRight; % sum of distances is the CKB severity index
                            CKBmin(i,j) = dLeft+dRight; 
                        end
                    end
                end
            end
            ...
            if (set(1,1,i) ~= set(1,1,j) ||...
                ...
                if (set(2,1,i) ~= set(2,1,j) ||...
                    ...
                    if choice(i) == 1 && ...
                        ...
                        right = [set(2,1,i),set(2,3,i);set(2,1,j),set(2,3,j)];
